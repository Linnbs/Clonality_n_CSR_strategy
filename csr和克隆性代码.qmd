...
分析流程包括： 1. **数据准备**: 加载必要的 R 包，导入并清理数据。 2. **克隆性与入侵性分析**: \* 分析克隆有无与 CSR 策略对入侵等级的综合影响。 \* 分析克隆有无与 CSR 策略对入侵范围的综合影响。 3. **克隆类型与入侵性分析**: \* 分析不同克隆类型（地上/地下）与 CSR 策略对入侵等级的综合影响。 \* 分析不同克隆类型（地上/地下）与 CSR 策略对入侵范围的综合影响。
